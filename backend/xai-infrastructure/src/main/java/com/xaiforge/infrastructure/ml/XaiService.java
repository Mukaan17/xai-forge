...
import com.xaiforge.infrastructure.ml.config.XaiConfig;
...
public class XaiService {
    ...
    public PredictionResponse predict(Long modelId, Map<String, String> inputData, Long userId) {
        try {
            MLModel mlModel = modelRepository.findByIdAndDatasetOwnerId(modelId, userId)
                .orElseThrow(() -> new com.xaiforge.common.exception.ModelNotFoundException(modelId));
    ...
    private Example<?> createExampleFromInput(Map<String, String> inputData, MLModel mlModel, Model<?> model) {
        ...
        if (mlModel.getModelType() == MLModel.ModelType.CLASSIFICATION) {
            return new ArrayExample<>(new org.tribuo.classification.Label("unknown"), featureNames, featureValues);
        } else {
            // For regression, create a Regressor example with dummy values
            String[] targetNames = mlModel.getTargetVariable().split(",");
            org.tribuo.regression.Regressor.DimensionTuple[] dims = new org.tribuo.regression.Regressor.DimensionTuple[targetNames.length];
            for (int i = 0; i < targetNames.length; i++) {
                dims[i] = new org.tribuo.regression.Regressor.DimensionTuple(targetNames[i].trim(), 0.0);
            }
            return new ArrayExample<>(new org.tribuo.regression.Regressor(dims), featureNames, featureValues);
        }
    }
    ...
    private PredictionResponse createPredictionResponse(Prediction<?> prediction, 
        ...
        if (modelType == MLModel.ModelType.CLASSIFICATION) {
            ...
            Prediction<org.tribuo.classification.Label> labelPrediction = (Prediction<org.tribuo.classification.Label>) prediction;
            org.tribuo.classification.Label predictedLabel = labelPrediction.getOutput();
            String predictedClassName = predictedLabel.getLabel();
            
            // Convert scores to proper probabilities using softmax normalization
            Map<String, org.tribuo.classification.Label> outputScores = labelPrediction.getOutputScores();
            
            // Calculate softmax normalization
            double sumExpScores = 0.0;
            Map<String, Double> expScores = new HashMap<>();
            
            for (Map.Entry<String, org.tribuo.classification.Label> entry : outputScores.entrySet()) {
            ...
            for (Map.Entry<String, Double> entry : expScores.entrySet()) {
                ...
                featureImportance.put(labelName, probability);
            ...
            return new PredictionResponse(predictedClassName, confidence, featureImportance);
        } else {
            @SuppressWarnings("unchecked")
            Prediction<org.tribuo.regression.Regressor> regressorPrediction = (Prediction<org.tribuo.regression.Regressor>) prediction;
            org.tribuo.regression.Regressor predictedRegressor = regressorPrediction.getOutput();
            ...
            return new PredictionResponse(String.valueOf(denormalizedValue), 1.0, featureImportance);
        }
    }
    ...
    private ExplanationResponse generateExplanation(Prediction<?> prediction, Example<?> example, 
        ...
        boolean isDegenerate = contributions != null && !contributions.isEmpty() &&
            contributions.stream().allMatch(c -> Math.abs(c.contribution()) < 1e-6);
        ...
        if (isDegenerate) {
            ...
            warnings.add(degenerateWarning);
        ...
        String predictionStr = getPredictionString(prediction, mlModel.getModelType());
        
        // Build summary
        String summary = explanationText;
        if (!warnings.isEmpty()) {
            summary = String.join(" ", warnings) + " " + explanationText;
        }
        
        // Build metadata
        Map<String, Object> metadata = new HashMap<>();
        metadata.put("prediction", predictionStr);
        ...
        return new ExplanationResponse(summary, contributions, metadata);
    }
     ...
     */
    private List<ExplanationResponse.FeatureImpact> generateLimeExplanation(
            MLModel mlModel, Example<?> example, List<String> featureNames, MLModel.ModelType modelType) {
        
        List<ExplanationResponse.FeatureImpact> contributions = new ArrayList<>();
     ...
     */
    private List<ExplanationResponse.FeatureImpact> extractRegressionWeights(
        ...
        try {
            ...
            for (Map.Entry<String, Double> entry : featureValues.entrySet()) {
                ...
                ExplanationResponse.FeatureImpact contrib = new ExplanationResponse.FeatureImpact(
                    featureName,
            ...
            if (modelAppearsDegenerate || contributions.isEmpty() || 
                (contributions.size() > 0 && contributions.stream().allMatch(c -> Math.abs(c.contribution()) < 1e-6))) {
            ...
            if (!contributions.isEmpty()) {
                double maxContribution = contributions.stream()
                    .mapToDouble(c -> Math.abs(c.contribution()))
                    .max()
                    .orElse(1.0);
                
                if (maxContribution > 0) {
                    log.debug("Normalizing contributions: max contribution = {}", maxContribution);
                    for (ExplanationResponse.FeatureImpact contrib : contributions) {
                        double normalized = Math.abs(contrib.contribution()) / maxContribution;
                        // Create new record with normalized contribution
                        int index = contributions.indexOf(contrib);
                        contributions.set(index, new ExplanationResponse.FeatureImpact(
                            contrib.feature(),
                            contrib.impact(),
                            contrib.direction(),
                            normalized
     ...
     * @return List of feature contributions based on actual model weights
     */
    private List<ExplanationResponse.FeatureImpact> extractClassificationWeights(
            Model<?> model, Map<String, Double> featureValues, MLModel mlModel) {
     ...
     * @return List of simplified feature contributions
     */
    private List<ExplanationResponse.FeatureImpact> generateFallbackExplanation(Example<?> example) {
        Map<String, Double> featureValues = new HashMap<>();
        for (Feature feature : example) {
            featureValues.put(feature.getName(), feature.getValue());
        }
    ...
    private String generateExplanationText(List<ExplanationResponse.FeatureImpact> contributions, 
        ...
        for (int i = 0; i < Math.min(3, contributions.size()); i++) {
            ExplanationResponse.FeatureImpact contrib = contributions.get(i);
            explanation.append(contrib.feature());
            explanation.append(" (");
            explanation.append(contrib.direction());
            explanation.append(" impact: ");
            explanation.append(String.format("%.2f", contrib.contribution()));
            explanation.append(")");
